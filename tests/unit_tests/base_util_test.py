...
from base_util import validate_config, parse_file_size, url_to_safe_filename, preprocess_url, extract_filename_from_url, to_safe_filename
...
@pytest.mark.parametrize("url, expected_fn", [
    (f"http://{DUMMY_DOMAIN}", DUMMY_DOMAIN.replace(".", "_")),  # no path so prefix based on DOMAIN in fn
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}", "forward"),  # return last path element as file name
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/", "forward"),  # same for trailing slash in path
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/;semicolons;", "forward"),  # url part after ; is put in url params
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/url%20encodings", "url%20encodings"),  # url encoded does end up in path
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/{DUMMY_FILE}", DUMMY_FILE)  # filenames at the end, returned "as is"
])
...
@pytest.mark.parametrize("unsafe_fn, safe_fn", [
...
@pytest.mark.parametrize("url, safe_fn", [
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/test漢file.mp3", "testfile.mp3"),  # non-ascii is filtered out
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/test file.mp3", "test_file.mp3"),  # spaces replaces with _
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/test%20file.mp3", "test_file.mp3"),  # first unquoted, the space replaced with _
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}/test_file.mp3/", "test_file.mp3"),  # trailing slash no problem
    (f"http://{DUMMY_DOMAIN}/{DUMMY_PATH}", "forward"),  # last path element is filename
    (f"http://{DUMMY_DOMAIN}/url%20encoded%20path", "url_encoded_path"),  # last path element is filename
])
