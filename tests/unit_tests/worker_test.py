...
DUMMY_RESULT = Result.from_json(json.dumps({
    "generator": {
        "id": "dummy-id-12345",
        "type": "Software",
        "name": "DOWNLOAD",
...
@pytest.mark.parametrize("file_exists, prior_result_saved, requires_download", [
    (True, True, False),
    (False, False, True),
    (False, True, True),
])
def test_requires_download(config, file_exists, prior_result_saved, requires_download):
    try:
        ...
        when(w)._save_prior_download_result(DUMMY_DOC, DUMMY_TASK).thenReturn(prior_result_saved)
        ...
        verify(w, times=1 if file_exists else 0)._save_prior_download_result(DUMMY_DOC, DUMMY_TASK)
    finally:
        unstub()